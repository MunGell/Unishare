...
var substreamData = db.execute('SELECT * FROM SUBSTREAMS WHERE substream_id=' + Titanium.UI.currentWindow.info.substream_id);
var substream = {};
	substream.id = substreamData.fieldByName('substream_id'); 
	substream.sensor_id = substreamData.fieldByName('substream_sensor'); 			// Sensor ID for DB
	substream.sensor_title = Titanium.App.Sensors[substream.sensor_id].title; 		// Sensor name
	...
	substream.initial_datastream_id = substream.datastream_id;						// Initial value to check edition later
...
var tableCreation = function()
{
	...
	var sensorRowLabel = Titanium.UI.createLabel({
		text: 'Sensor: ' + substream.sensor_title,
		left:10,
		width:'auto',
		bottom:5,
		height: 'auto',
	...
	var deleteSection = Titanium.UI.createTableViewSection();
	var deleteRow = Ti.UI.createTableViewRow({
		height: 50,
		backgroundColor: '#FFFFFF'
	});
	var deleteRowView = Titanium.UI.createView({
		height: 'auto',
		layout:'vertical',
		top:5,
		right:5,
	...
	var deleteRowLabel = Titanium.UI.createLabel({
		text: 'Delete',
		left:10,
		width:'auto',
		bottom:5,
	...
	deleteButton.addEventListener('click', function(){
		// DB Connect
		var db = Titanium.Database.open('unishare');
		db.execute('DELETE FROM SUBSTREAMS WHERE substream_id=' + substream.id);
		Titanium.App.streamView();
		Titanium.UI.currentWindow.close();
	});
	deleteRowView.add(deleteRowLabel);
	deleteRow.add(deleteRowView);
	deleteRow.add(deleteButton);
	deleteSection.add(deleteRow);
	
	// Add everything to the table
	substreamTable.data = [datastreamRow, sensorRow, deleteSection];
}
...
done.addEventListener("click", function()
{
	// DB Connect
	var db = Titanium.Database.open('unishare');
	var existData = db.execute('SELECT * FROM SUBSTREAMS WHERE substream_ds=' + substream.datastream_id + ' AND stream_id=' + stream.id);
		if(existData.rowCount == 0 || (substream.datastream_id == substream.initial_datastream_id))
		{
			db.execute('UPDATE SUBSTREAMS SET substream_name=?, substream_sensor=?, substream_ds=? WHERE substream_id=?', substream.datastream_title, substream.sensor_id, substream.datastream_id, substream.id);
			db.close();
			Titanium.App.streamView();
			Titanium.UI.currentWindow.close();
		}
		else
		{
			var alert = Titanium.UI.createAlertDialog({
				title: "Substream is already configured",
				message: "Substream " + substream.datastream_title + 'is already exists.'
			});
			alert.show();
		}
	db.close();
